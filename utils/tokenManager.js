...
function tokenPath(mallId, shopNo=1){ const dir = path.join(DATA_DIR, mallId); ensureDir(dir); return path.join(dir, `${shopNo}.json`); }
function legacyPath(mallId){ return path.join(LEGACY_DIR, `${mallId}_token.json`); }

// expires_at 5분 전부터 만료 간주
function isExpired(t, safetySeconds=300){ if(!t?.expires_at) return true; return Date.now() >= new Date(t.expires_at).getTime() - safetySeconds*1000; }
